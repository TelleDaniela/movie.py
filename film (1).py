...
def addMovie(ws,file_path):
    ...
    a = duration.split(':')
    if int(a[0]) > 99 or int(a[0]) < 0:
        print('wrong hour count')
        return
    if int(a[1]) > 60 or int(a[1]) < 0:
        print('wrong minute count')
        return
    if int(a[2]) > 60 or int(a[2]) < 0:
    ...
    for row in range(2, maxrow):
        if not ws.cell(row, 1).value:     
            ws['a'+str(row+1)] = row
            ws['a'+str(row+1)].alignment = Alignment(horizontal='left',vertical='center')
            ws['b'+str(row+1)] = name
            ws.column_dimensions['B'].width = 15
            ws['b'+str(row+1)].alignment = Alignment(horizontal='center',vertical='center')
            ws.column_dimensions['C'].width = 30
            ws['c'+str(row+1)] = description
            ws['d'+str(row+1)] = watched
            ws.column_dimensions['D'].width = 15
            ws['d'+str(row+1)].alignment = Alignment(horizontal='left',vertical='center')
            ws['e'+str(row+1)] = rating
            ws['e'+str(row+1)].alignment = Alignment(horizontal='center',vertical='center')
            ws['f'+str(row+1)].number_format = 'hh:mm:ss'
            ws['f'+str(row+1)] = duration
